...
const OrdenF = () => {
  const [oldIdArticulo, selOrdenF] = useState('');
  ...
  const handleSearchChange = async (e) => {
    ...
    if (search.length > 2) {
      try {
        ...
        const response = await axios.get(`http://192.168.1.145:5000/autocomplete2?search=${search}`);
  ...
  return (
    <div className="max-w-4xl mx-auto mt-10 p-6 bg-white shadow-md rounded-md">
      {/* <h1 className="text-xl font-bold mb-4">Orden de Fabricacion</h1>
      <form className="space-y-4" onSubmit={handleSubmit}>
        <div className="flex space-x-4">
          <div className="w-1/2 relative">
            <label className="block text-sm font-medium text-gray-700">Buscar Orden:</label>
            <input
              type="text"
              value={oldIdArticulo}
